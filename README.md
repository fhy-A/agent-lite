...
服务端现已具备独立的持久化只读 AgentRun：选择“只读分析”权限后，一次任务会由服务端连续完成模型请求与 `list_files`、`read_file`、`search_files`、`glob_files` 四个只读工具调用，浏览器只投影持久事件和子模型 SSE。页面关闭后任务仍可继续，重新打开时会按 `agentRunId + 事件游标` 恢复；服务重启后则等待前端重新注入内存态凭据。计划、接受编辑和自动模式继续使用原浏览器 Agent 循环，写入、命令、问卷与子任务尚未迁移。详见[服务端 Agent 循环迁移计划](docs/SERVER_AGENT_LOOP_PLAN.md)。
