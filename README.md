...
每个模型任务都拥有独立的运行 ID、消息队列、停止信号和恢复检查点。主任务输出期间再次发送的消息会作为同会话后台 AgentRun 独立运行，结果与用量只合并一次。主任务和后台消息分别从发送时刻记录端到端耗时，刷新后沿用原起点；结果按完成顺序显示，后台回答通过可点击的单行回复引用关联原用户消息，不在正文重复完整提示词。页面刷新后会重新连接各自的 SSE 事件流；切换或新建会话只改变前台视图，不会取消后台任务。
