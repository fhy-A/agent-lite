...
服务端现已具备独立的持久化 AgentRun：选择“只读分析”权限后，一次任务会由服务端连续完成模型请求与项目读取工具调用；遇到关键决策时可持久暂停问卷，并在浏览器提交答案后继续原运行。AgentRun 协议也已支持网络、Skills、命令、项目记忆、编辑提案、直接文件变更和子任务。每个 `task` 会创建独立持久 Child AgentRun，继承父任务模型、权限和工具策略但不能再次委派或打开问卷；同一模型轮次最多并发运行 3 个子任务，超出的调用排队，最终工具结果仍按模型原始调用顺序回填。子任务授权由父任务统一展示和提交，父任务取消会同步停止子任务，服务重启后按原子任务 ID 复用结果和用量。页面关闭后任务仍可恢复，重新打开时会按 `agentRunId + 事件游标` 续接；服务重启后则等待前端重新注入内存态凭据。计划、接受编辑和自动模式目前仍使用原浏览器 Agent 循环，下一阶段将切换并验证正式单一执行所有权。详见[服务端 Agent 循环迁移计划](docs/SERVER_AGENT_LOOP_PLAN.md)。
