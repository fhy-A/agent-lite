# Code TODO  _上次更新：2026-07-18 05:16_
...
- [ ] **完整 Agent 循环下沉服务端** — 已完成结构化模型轮次结果和首批只读工具注册表；下一步按 [`docs/SERVER_AGENT_LOOP_PLAN.md`](docs/SERVER_AGENT_LOOP_PLAN.md) 建立可消费注册表的持久化 `AgentRun` 状态机，再迁移权限/问卷、写入工具和子任务调度，使浏览器关闭后任务仍可运行。
