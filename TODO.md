# Code TODO  _上次更新：2026-07-20_

> 只记录尚未完成且可以继续执行的事项；已完成内容统一查阅 `CHANGELOG.md`。

## P0 · 上线前

- [ ] **固定平台连接地址** — Base URL 与登录认证地址使用部署后的中转站地址，前端不再展示或允许自定义。
- [ ] **准备上线资料** — 确认服务器、域名、HTTPS、备份、监控、文档与合规要求。
- [x] **注册邮箱通知模板** — 完成中转站注册与验证邮件的内容和发送配置。✅ 2026-07-19
- [ ] **接入充值系统** — 已对接支付FM 免签（易支付协议），待完成：找人扫码实测 ngrok 回调闭环、支付FM 商户名称改为品牌名。
- [ ] **获取营业执照** — 申请个体工商户营业执照，用于支付宝/微信官方商户资质。
- [ ] **支付渠道升级** — 拿到执照后：签约模式（去浮动）或直接支付宝 API（复用 Mock 支付页面）。

## P1 · 核心能力

- [ ] **补充 Skill 输出证据校验** — 对性能类方案中的未测量数字、阈值和性能排序增加结构化检查，不再只依赖提示词约束。
- [ ] **继续 `app.js` 拆分阶段 4** — 下一批盘点并抽离会话状态、持久化与分支领域，保持前台查看会话和后台运行会话相互隔离。
- [ ] **收口剩余 i18n** — 合并 `src/core/i18n.js` 内 `LANG` / `I18N` 的重复键，并评估服务端错误字典是否采用统一键约定。

## P2 · 后续路线

- [ ] **Agent 规划任务能力** — 支持复杂任务自动拆解、依赖排序和分阶段执行。
- [ ] **中转站深度整合** — 打通 Token 用量统计、统一模型选择器和 Key 管理闭环。
- [ ] **macOS / Linux 支持** — 适配 shell、进程管理、托盘和安装流程。
- [ ] **客户端应用** — 评估并开发 Windows / macOS 原生客户端。
- [ ] **营销获客** — 规划推广渠道、内容与 SEO。
