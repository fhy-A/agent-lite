...
- **app.js** owns prompt/context assembly, UI projection, and questionnaire/authorization interactions. All four permission profiles delegate new main tasks to the server. A message sent while the same session is already streaming creates a separate durable background AgentRun and is restored from a session background checkpoint. Persisted legacy browser checkpoints fail explicitly without replay; the old browser loop has no production entry and remains only as unreachable code pending deletion.
