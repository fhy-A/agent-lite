...
The production UI exposes a `read` permission profile as the explicit ownership switch. Its session checkpoint stores `executionOwner`, `agentRunId`, `agentEventCursor`, the active child `runtimeRunId`, and visible questionnaire/authorization state; model rounds reuse the existing SSE renderer while durable model/tool events are projected into the session in order. Cursor advancement and the projected message snapshot are persisted together. A restored authorization card can submit its decision without an in-memory Promise and then resume the same AgentRun. The server registry and AgentRun execute `web_fetch`, `use_skill`, `read_skill_resource`, `run_command`, `save_memory`, `write_file`, and `delete_file` without browser relay; Skill resources are confined to packaged resource directories. Commands are selected only for `accept`/`bypass`: `accept` persists approval before execution, output tails are checkpointed while running, cancellation terminates the active process tree, and a restarted `running` record becomes an `unknownState`/`notReplayed` tool result. Project memory is also limited to `accept`/`bypass`; its shared HTTP/AgentRun service atomically replaces the target and treats identical saved content as a replay, so recovery after a write-before-checkpoint crash does not rewrite the file. Direct file mutations use the same permission filter: `accept` persists authorization before execution, writes use atomic replacement plus a stable backup, and deletes persist a stable backup and operation receipt before removal so restart recovery can distinguish a completed delete from a missing target. Production `plan`, `accept`, and `bypass` profiles remain browser-owned only because sub-agents still lack a durable server or explicit client-delegation protocol. Whole-task ownership must not switch until that boundary is complete.
