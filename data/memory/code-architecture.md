...
- **app.js** owns prompt/context assembly, UI projection, questionnaire/authorization interactions, and the legacy browser Agent loop used only by the automatic (`bypass`) profile. Read, plan, and accept tasks delegate their full multi-round loop to the server.
...
- Durable AgentRun state is stored separately under `data/agent-runs/`; production read/plan/accept session checkpoints reference it without copying credentials into session data.
