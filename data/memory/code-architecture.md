...
- **app.js** owns prompt/context assembly, UI projection, and questionnaire/authorization interactions. All four permission profiles delegate new main tasks to the server. Persisted legacy browser checkpoints now fail explicitly without replay; the old browser loop remains only for a temporary background subtask created when another message is sent while the same session is already streaming.
