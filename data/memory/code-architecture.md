...
The production UI exposes a `read` permission profile as the explicit ownership switch. Its session checkpoint stores `executionOwner`, `agentRunId`, `agentEventCursor`, the active child `runtimeRunId`, and visible questionnaire/authorization state; model rounds reuse the existing SSE renderer while durable model/tool events are projected into the session in order. Cursor advancement and the projected message snapshot are persisted together. A restored authorization card can submit its decision without an in-memory Promise and then resume the same AgentRun. Although the server protocol and UI now support `plan`, `accept`, and `bypass` edit authorization semantics, those production profiles remain browser-owned because their current tool policy also includes network, Skills, commands, direct writes/deletion, and sub-agents. Whole-task ownership must not switch until those tools have durable server or explicit client-delegation protocols.
