...
│   │   ├── keywords: (recommended, comma-separated)
│   │   └── tools: (recommended, comma-separated)
...
Any example files and directories not needed for the skill should be deleted. The initialization script creates example files in `scripts/`, `references/`, and `assets/` to demonstrate structure, but most skills won't need all of them. `quick_validate.py` rejects these scaffold resources when they are left unchanged, so implement or remove every example before packaging.
