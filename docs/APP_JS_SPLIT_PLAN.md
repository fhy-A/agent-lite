...
下一批继续阶段 3，但只拆消息分组、投影与渲染：

1. 盘点 `renderMessages()`、思考摘要、工具轮次、最终回答、后台回复引用和 usage 行所依赖的消息字段、稳定顺序与 DOM 回调。
2. 新增 `src/ui/messages.js`，先迁移消息分类、分组和 HTML 投影，不同时调整流式协议、会话状态或滚动策略。
3. Markdown、Diff、工具卡片、时间戳与状态条通过显式渲染接口注入；事件绑定、增量流式补丁、自动滚动和会话切换继续由现有所有者负责。
4. 增加用户/助手/工具结果、思考与最终回答分离、后台回复、编辑建议去重、流式占位和刷新重放的回归测试。
5. 完成语法、定向、全量和浏览器冒烟验证后单独提交，再继续时间线模块。
