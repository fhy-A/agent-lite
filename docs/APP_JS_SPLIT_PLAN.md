...
- [ ] 抽离 Diff 解析和编辑建议卡片到 `src/ui/diff.js`。
...
下一批继续阶段 3，但只拆 Diff 解析与编辑建议卡片：

1. 盘点 `normalizeDiffText()`、统计、文件路径提取、折叠和编辑建议卡片所依赖的纯函数与 DOM 回调。
2. 新增 `src/ui/diff.js`，先迁移 Diff 解析、统计与 HTML 生成，不同时调整卡片样式、应用状态或消息顺序。
3. 文件写入审批、已应用状态、复制、展开和路径打开动作继续由现有所有者负责，通过显式回调接入新模块。
4. 增加新增/删除/重命名、多文件、截断、危险 HTML、空 Diff 和旧定义禁入回归测试。
5. 完成语法、定向、全量和浏览器冒烟验证后单独提交，再继续消息投影模块。
