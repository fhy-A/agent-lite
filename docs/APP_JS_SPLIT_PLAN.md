...
下一批进入阶段 3，但只拆低风险的 Markdown、代码块与 ANSI 渲染：

1. 盘点 Markdown 解析、代码高亮、代码块操作和 ANSI 转义所依赖的纯函数与 DOM 回调。
2. 新增 `src/ui/markdown.js`，先迁移渲染与解析，不同时调整样式或流式消息结构。
3. 消息列表、流式投影、滚动和会话状态继续留在 `app.js`，只通过渲染接口取得 HTML。
4. 增加危险 HTML、代码围栏、语言识别、ANSI 和旧定义禁入回归测试。
5. 完成语法、定向、全量和浏览器冒烟验证后单独提交，再继续 Diff 模块。
