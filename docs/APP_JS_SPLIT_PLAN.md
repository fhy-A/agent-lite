...
> 状态：进行中（阶段 2 已完成三项，下一项为设置、更新和认证）
> 适用范围：Code 前端  
...
1. 盘点设置页、版本更新、平台连接与认证相关状态、DOM 节点和 API。
2. 新增 `src/features/settings.js`，通过显式依赖注入承接设置面板与相关事件。
3. 保留模型、权限和 AgentRun 等执行链所有权，不在本批改变协议或产品行为。
4. 增加模块加载顺序、公开接口、旧定义禁入与设置行为回归测试。
5. 完成语法、定向、全量和浏览器冒烟验证后单独提交。
