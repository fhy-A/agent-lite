...
> 状态：进行中（阶段 2 已完成，阶段 3 下一步抽离顶部面板与 Session Info）
> 适用范围：Code 前端  
...
下一批继续阶段 3，但只拆时间线、分支与压缩标记：

1. 盘点 `renderTimeline()`、分支流标记、上下文压缩标记和时间线点击定位所依赖的消息字段、会话元数据与 DOM 事件。
2. 新增 `src/ui/timeline.js`，迁移时间线数据投影与标记 HTML；消息主投影继续只通过显式回调使用分支/压缩标记。
3. 时间线容器更新、点击定位和分支操作通过公开接口装配，不同时调整消息顺序、会话持久化、流式协议或滚动策略。
4. 增加普通会话、分支边界、隐藏工具消息、压缩摘要、时间线定位和刷新恢复回归测试。
5. 完成语法、定向、全量和浏览器冒烟验证后单独提交，再继续顶部面板模块。
