...
| 工具注册与执行编排 | 全部工具及有界并发子任务均已在服务端；正式变更档位尚未切换 | 服务端 |
...
- [待完成] 将正式 `plan` / `accept` 任务切换为 AgentRun 单一执行所有者，复用现有问卷、授权和流式投影，并完成刷新、切换会话、停止任务与服务重启端到端验证。
