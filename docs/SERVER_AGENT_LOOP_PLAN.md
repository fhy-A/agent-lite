...
截至 2026-07-18，服务端同时具备两层运行时：单次模型轮次由 `/api/runtime/runs` 持有；独立的 `/api/agent/runs` 持久化多轮消息，并能按任务工具策略自动执行项目读取、网络、Skills、命令、项目记忆、直接文件变更和顺序子任务。每个 `task` 都会创建独立持久 Child AgentRun，记录父运行与父工具调用，继承模型、权限和工具策略但移除再次委派与交互问卷；子任务授权经父任务代理，取消向子任务传播，完成结果和用量只合并一次，服务重启后复用原 Child AgentRun。正式前端能够投影问卷与编辑 diff 审查卡片、跨刷新恢复并提交决定。由于同一模型轮次的多个子任务还没有服务端有界并发协议，“只读分析”仍是当前唯一正式所有权开关，其余档位继续由浏览器端 `runAgentLoop()` 编排。
...
只读 AgentRun 已成为正式界面的可选产品能力，并已接管无副作用的问卷等待；服务端编辑、命令、直接写入与删除协议已完成并经过独立 API 验证，但尚未接管正式界面的变更任务。完成子任务迁移、授权界面投影和单一所有权切换前，不能同时让浏览器与 AgentRun 消费同一工具调用。
...
| 工具注册与执行编排 | 项目读取、网络、Skills、命令、记忆写入、直接写入/删除、编辑提案/应用及顺序子任务在服务端；正式同轮并发子任务仍在浏览器 | 服务端 |
| 结构化问卷等待状态 | 服务端状态机；浏览器只提交答案 | 服务端状态机；浏览器只提交答案 |
| 副作用权限决定 | 编辑授权状态和卡片投影已完成，常规编辑任务仍由浏览器编排 | 服务端状态机；浏览器只提交决定 |
| 子任务调度 | 服务端已支持持久顺序 Child AgentRun；正式同轮并发仍在浏览器 | 服务端 |
| 会话投影与界面渲染 | 浏览器 | 浏览器 |
...
- `task` 已作为 `effect=delegation`、`idempotent=true`、`background=true` 加入注册表；仅 `plan` / `accept` / `bypass` 可选择。当前按父工具调用顺序创建独立持久 Child AgentRun，同一模型轮次的有界并发调度留待下一阶段。
...
### 阶段 4：前端投影、问卷和编辑授权闭环已完成；正式写入所有权待切换
...
- [已完成] 建立顺序子任务的持久 Child AgentRun、父子关系、权限继承、授权代理、取消传播、用量合并和重启复用协议。
- [待完成] 支持同一模型轮次多个 `task` 调用的有界并发与确定性结果排序；覆盖齐全后再切换整次任务的单一执行所有权，不能通过串行化造成正式能力或响应时间静默回退。
- [待完成] 为 `bypass` 建立明确的产品入口与正式前端端到端验证。

### 阶段 5：子任务与前端瘦身（顺序持久协议已完成）

- [已完成] 子 Agent 迁移到服务端持久调度器，继承主任务的模型、权限和工具策略且不能自行提升权限或嵌套委派。
- [待完成] 对同轮多个子任务实施有界并发，保持模型工具调用顺序与父消息协议稳定。
