...
3. 同一会话已有主任务输出时再次发送的消息，暂时仍使用旧浏览器循环执行临时后台子任务，这是删除旧循环前唯一待迁移的执行入口；
...
| Agent 消息构造 | 新主任务在服务端；同会话后台消息暂由浏览器执行 | 服务端 |
...
- [待完成] 将“同一会话输出中再次发送消息”的临时后台子任务迁移到服务端，再从 `app.js` 删除旧 Agent 编排逻辑，只保留任务提交、事件投影、用户决策和本地交互。
