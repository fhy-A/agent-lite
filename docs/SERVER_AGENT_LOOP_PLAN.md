...
截至 2026-07-18，服务端同时具备两层运行时：单次模型轮次由 `/api/runtime/runs` 持有；独立的 `/api/agent/runs` 持久化多轮消息，自动执行首批四个只读工具，并通过 `waiting_user_input` 持久等待结构化问卷答案。AgentRun 也已具备编辑提案、`waiting_authorization` 持久授权和幂等应用协议；正式前端能够把该等待状态投影成 diff 审查卡片、跨刷新恢复并提交决定。由于计划/接受编辑档位仍包含尚未迁移的网络、Skills、命令和子任务工具，“只读分析”仍是当前唯一正式所有权开关，其余档位继续由浏览器端 `runAgentLoop()` 编排。
...
只读 AgentRun 已成为正式界面的可选产品能力，并已接管无副作用的问卷等待；服务端编辑授权协议已完成并经过独立 API 验证，但尚未接管正式界面的写入任务。完成授权界面投影和单一所有权切换前，不能同时让浏览器与 AgentRun 消费同一编辑调用；命令、删除和子任务也仍保留在浏览器链路。
...
| 工具注册与执行编排 | 四个只读工具及编辑提案/应用协议在服务端；正式编辑、命令等仍在浏览器 | 服务端 |
...
- 后续命令、删除、网络与子任务工具将在各自的权限和幂等协议确定后加入注册表。
...
- [待完成] 迁移 `plan` / `accept` 仍依赖的网络、Skills、命令、直接写入/删除和子任务能力，或为这些能力建立明确的持久客户端委托协议；覆盖齐全后再切换整次任务的单一执行所有权，不能通过缩减工具列表制造静默能力回退。
