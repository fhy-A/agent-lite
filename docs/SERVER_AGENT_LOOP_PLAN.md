...
截至 2026-07-18，服务端同时具备两层运行时：单次模型轮次由 `/api/runtime/runs` 持有；独立的 `/api/agent/runs` 已能持久化多轮消息，并自动执行首批四个只读工具。后者尚未接入正式 `app.js`，因此当前用户界面仍由浏览器端 `runAgentLoop()` 编排完整任务。
...
独立 AgentRun 接口已经验证浏览器不轮询时的只读多轮续跑；但在正式前端切换执行所有权之前，不能把该能力宣称为默认产品行为。
...
| Agent 消息构造 | 服务端 AgentRun 已实现；正式 UI 仍在浏览器 | 服务端 |
| 工具注册与执行编排 | 四个只读工具可由服务端执行；正式 UI 仍在浏览器 | 服务端 |
...
- 当前状态包含 `model`、`tools`、`waiting_credentials`、`completed`、`failed`、`cancelled`；权限和问卷等待状态在阶段 4 引入。
...
- API Key 仅保留在内存；服务重启后非终态任务转为 `waiting_credentials`，通过恢复端点重新注入凭据，从持久化的 `model` 或 `tools` 检查点继续。
- 正式前端尚未切换到 AgentRun，避免迁移期间同一个工具调用同时被浏览器和服务端消费。

### 阶段 4：权限、问卷与写入

- 先让前端保存 `agentRunId` 并投影 AgentRun 事件，使用显式能力开关完成单一执行所有权切换。
...
持久化记录包含请求选项、消息、轮次、工具检查点和事件，但不包含 API Key。显式 Base URL 禁止嵌入用户名或密码，Payload 顶层及嵌套模型选项中的凭据字段会被拒绝。当前 AgentRun 只会向模型暴露注册表中同时满足 `effect=read`、`idempotent=true`、`background=true` 的工具。
