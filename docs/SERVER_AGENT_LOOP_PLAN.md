...
截至 2026-07-18，服务端同时具备两层运行时：单次模型轮次由 `/api/runtime/runs` 持有；独立的 `/api/agent/runs` 持久化多轮消息，并能按任务工具策略自动执行项目读取、网络、Skills、命令、项目记忆、直接文件变更和子任务。每个 `task` 都会创建独立持久 Child AgentRun，记录父运行与父工具调用，继承模型、权限和工具策略但移除再次委派与交互问卷；同一模型轮次最多同时运行 3 个子任务，空位释放后再启动排队调用，父模型工具结果严格按原调用顺序回填。子任务授权经父任务代理，取消向子任务传播，完成结果和用量只合并一次，服务重启后复用原 Child AgentRun。正式界面的 `read`、`plan`、`accept` 和 `bypass` 新主任务均已切换到服务端单一执行所有权；同一会话在主任务输出期间再次发送的消息也会创建独立持久 AgentRun，以稳定 `clientRequestId` 实现创建幂等，并由会话后台检查点恢复结果、用量和授权。迁移前明确保存为 `executionOwner=browser` 的活动检查点保留已有消息并明确失败，不再自动重放可能已经发生的副作用。旧浏览器 Agent 循环、工具执行、子任务委派和消息队列代码已经删除，前端只保留任务提交、SSE/事件投影、用户决策与本地交互。

正式界面目前的执行边界如下：

1. 四种权限均提交任务级 AgentRun，服务端按权限档位独立完成模型与工具续轮；接受编辑必要时持久暂停授权，自动模式按既有语义直接执行允许操作；浏览器按游标投影事件，并直接附着当前子模型运行时保持原有流式体验；
2. 迁移前已保存且明确记录浏览器所有权的活动检查点停止流式状态、保留已有消息并写入明确失败说明，不再调用模型或工具；
3. 同一会话已有主任务输出时再次发送的消息会创建独立 AgentRun；主任务与后台任务分别持久化、投影和统计，刷新后按后台检查点重新附着，互不抢占执行状态；
4. 主任务入口和“继续”操作只允许进入服务端 AgentRun，不做浏览器自动降级或双重消费；
5. 刷新、切换会话和页面关闭不会取消 AgentRun，恢复时按持久游标重放尚未投影的事件。

AgentRun 已接管正式界面的全部新任务。计划提案保持只预览、不落盘；接受编辑的编辑、直接写入、删除和命令均由服务端持久暂停授权，浏览器仅展示真实动作与目标并提交决定；自动模式沿用无需授权的产品语义，但副作用仍经过相同的服务端路径、命令和恢复安全检查。任何任务只会进入一条所有权链路，禁止浏览器与 AgentRun 同时消费同一工具调用。
...
| Agent 消息构造 | 新主任务与同会话后台消息均在服务端 | 服务端 |
| 工具注册与执行编排 | 全部新任务、工具及有界并发子任务均在服务端 | 服务端 |
...
- 正式前端的四种权限均已显式切换到 AgentRun；恢复时优先遵守检查点保存的所有者，避免同一个工具调用同时被浏览器和服务端消费。
...
- [已完成] `bypass` 复用界面现有“自动”入口并切换到 AgentRun；人工验证直接写入/删除不弹授权、长命令刷新后续接、停止命令后不再恢复，自动化完整回归为 `537 passed, 25 subtests passed`。

### 阶段 5：子任务与前端瘦身（执行入口迁移已完成）

- [已完成] 子 Agent 迁移到服务端持久调度器，继承主任务的模型、权限和工具策略且不能自行提升权限或嵌套委派。
- [已完成] 对同轮多个子任务实施并发上限 3 的调度，排队调用在空位释放后启动，模型工具结果保持原调用顺序。
- [已完成] 迁移前 `executionOwner=browser` 活动检查点保留已有消息、停止流式状态并明确失败；不再自动重放模型或工具，也不会把任务中途切换给服务端。
- [已完成] 将“同一会话输出中再次发送消息”的临时后台任务迁移为独立持久 AgentRun；以稳定 `clientRequestId` 幂等创建，背景检查点保存模型参数、截止时间、发送/启动时间和 AgentRun ID，刷新或服务重启后重新附着；结果和用量去重合并，后台授权不覆盖主任务检查点，且后台任务不暴露问卷或再次委派工具。主任务与后台任务按各自发送时刻计算端到端耗时，结果按完成顺序呈现；后台结果通过稳定任务 ID 显示可点击回复引用，而不重复完整用户提示词。
- [已完成] 从 `app.js` 删除已无正式入口的旧 Agent 循环、浏览器工具执行、子任务委派、重复工具控制和消息队列，只保留任务提交、子模型 SSE 投影、AgentRun 事件投影、用户决策和本地交互；源码结构测试明确禁止旧入口回归。
- [已完成] 将旧循环中的工具图片视觉桥接迁到 AgentRun：`read_file` 的视觉结果以轻量持久标记记录，模型请求时再展开为 `image_url`，避免在 Agent 消息里重复持久化 base64，同时保留刷新与服务重启后的视觉上下文。
- [已完成] 通过自动化与分阶段人工验证覆盖刷新/浏览器离开、服务重启凭据恢复、多会话独立运行、取消、问卷以及编辑/写入/删除/命令副作用恢复。
