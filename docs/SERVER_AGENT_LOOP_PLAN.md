...
| Agent 消息构造 | 全部新任务在服务端；旧浏览器检查点保留兼容恢复 | 服务端 |
...
- [待完成] 为迁移前 `executionOwner=browser` 检查点制定兼容过期/转换策略，再从 `app.js` 删除旧 Agent 编排逻辑，只保留任务提交、事件投影、用户决策和本地交互。
